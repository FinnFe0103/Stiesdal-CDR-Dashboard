...
def render(app: Dash, data: pd.DataFrame) -> html.Div:
    ...
    @app.callback(
        Output(_00_own_ids.S_CHART, "children"),
    ...
    def update_bar_chart(methods: list[str], start_date, end_date) -> html.Div:
        ...
        # filter dataframe to exclude "Aggregate Supplier (Marketplace Report)" as a Supplier
        filtered_data = filtered_data[~filtered_data["Supplier"].isin(["Aggregate Supplier (Marketplace Report)"])]

        #if filtered_data.shape[0] == 0:
        #    return html.Div("No data selected.", id=_00_own_ids.S_CHART)

        # creating the relevant graph "TOP_TEN_SUPPLIERS"
        fig = _04_bar_plot.build_plot(filtered_data[["Supplier", "Tons Purchased/Sold"]], h=1)

        return html.Div(dcc.Graph(figure=fig), id=_00_own_ids.S_CHART)

    return html.Div(id=_00_own_ids.S_CHART)
