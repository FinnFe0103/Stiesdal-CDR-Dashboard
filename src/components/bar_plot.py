...
def build_plot(df, h=0):
    ...
    if h==0:
        fig = px.bar(top_ten, x="x_s", y=y, text_auto=".3s", title=title, height=600, width=960, hover_name=x, hover_data={"x_s": False, x: False, y: ':.2fk'}) # type: ignore 
        fig.update_layout(showlegend=False, xaxis_title=x) 
        fig.update_traces(marker_color="#00497a")
    else:
        top_ten.sort_values(y, ascending=True, inplace=True)
        fig = px.bar(top_ten, y="x_s", x=y, text_auto=".3s", title=title, height=600, width=960, hover_name=x, hover_data={"x_s": False, x: False, y: ':.2fk'}) # type: ignore
        fig.update_traces(marker_color="#00497a")
