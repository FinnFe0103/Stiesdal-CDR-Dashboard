...
def render(app: Dash, data: pd.DataFrame) -> html.Div:
    ...
    @app.callback(
        Output(own_ids.P_CHART, "children"),
    ...
    def update_bar_chart(methods: list[str], start_date, end_date) -> html.Div:
        ...
        if filtered_data.shape[0] == 0:
            return html.Div("No data selected.", id=own_ids.P_CHART)

        # creating the relevant graph "TOP_TEN_SUPPLIERS"
        fig = bar_plot.build_plot(filtered_data[["Purchaser", "Tons Purchased"]], h=1)

        return html.Div(dcc.Graph(figure=fig), id=own_ids.P_CHART)

    return html.Div(id=own_ids.P_CHART)
